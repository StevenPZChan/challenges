...
def _load_words():
    with open('../dictionary.txt', 'r') as f:
        with open('../' + f.read(), 'r') as fi:
            return set([word.strip().lower() for word in fi.read().split()])
...
LETTER_SCORES = dict(zip([letter.name for letter in distribution], [int(letter.value) for letter in distribution]))
